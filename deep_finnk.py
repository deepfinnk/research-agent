...
try:
    ...
    print("Warning: owly.utils not found. Assuming DocumentProcessingToolkit is standard or placeholder.")
...
class DeepFinnk:
    ...
    def __init__(self):
        ...
        # self.document_toolkit = DocumentProcessingToolkit(model=self.models["document"])
        # self.file_write_toolkit = FileWriteToolkit(output_dir="./")
        self.all_tools = [
            ...
            *self.excel_toolkit.get_tools(),
            # *self.document_toolkit.get_tools(),
            # *self.file_write_toolkit.get_tools(),
        ]
    ...
    def query(self, prompt:str):
        ...
        try:
            ...
            print(f"Error: Failed to decode JSON from initial response: {init_response_str}")
            return None
        ...
        if "points" in data and isinstance(data["points"], list):
            ...
            for point in points_list:
                ...
                try:
                    deep_result_answer = self.deep_search(
                        prompt=point,
                    )
                    print(f"--- Result for Point '{point}': ---")
                    deep_result_answer = [item['assistant'] for item in deep_result_answer if 'assistant' in item]
                    deep_results.append({
                        "point": point,
            ...
            combined_results = "\n\n".join([f"Point: {item['point']}\nResult:\n{item['result']}" for item in deep_results])
    ...
    def _construct_society(self, question: str) -> RolePlaying:
        ...
        assistant_agent_kwargs = {"model": self.models["assistant"], "tools": self.all_tools}
...
if __name__ == "__main__":
    df = DeepFinnk()
    final_result = df.query(prompt="I want to buy a house in Delft, The Netherlands. What should I do?")
